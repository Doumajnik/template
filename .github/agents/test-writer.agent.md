...
### Step 2 — Write tests for each function (be thorough — adversarial mindset)

For **every** public function, write tests in **all 12 categories below**. Minimum **20 tests per function** (typical: 25–40). Quality + coverage of categories matters more than raw count, but anything below 20 means you skipped categories.
...
**1. Happy path (3+ tests)** — typical, realistic inputs across the documented input space. Verify exact outputs, not just "not None".
...
**3. Boundary values (3+ tests)** — zero, one, exactly-the-limit, one-below-the-limit, one-above-the-limit, max integer, min integer, empty string, single character, single-element list, max documented size.
...
**8. Error contract (3+ tests)** — every documented exception type is raised under its documented condition, error messages contain the documented context (e.g., the offending value), errors are NOT silently swallowed (no `except: pass` slipping through), no leaking of internal details.
...
- **Minimum 20 tests per function**, distributed across the 12 categories above. Functions with side effects, string handling, or state typically need 30–40.
