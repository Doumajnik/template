...
- **GREEDY_MODE: OFF** — When ON, the Orchestrator runs the **Super Greedy Pipeline** instead of the full Planning Sequence. Designed for users with **unlimited LLM resources** (e.g., Claude Code unlimited plans). Enables: LLM Council multi-model consensus, N-version programming for critical paths, continuous security/quality audits after EVERY agent, mutation testing, adversarial red-teaming, cross-file coherence reviews, and multi-model dispatch (same task to 2–3 models, synthesize best). **Mutually exclusive with BUDGET_MODE.** GREEDY_MODE forces DEEP_MODE ON. Test floor: ≥25 per function, ≥100 per functionality. See [AGENTS.md → Super Greedy Pipeline](../AGENTS.md#super-greedy-pipeline-greedy_mode-on) for the full step list.
